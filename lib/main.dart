...
class _AdminSummaryGrid extends StatelessWidget {
 ...
 @override Widget build(BuildContext context)=>LayoutBuilder(builder:(context,c){final w=c.maxWidth;final columns=w>850?3:w>520?2:1;final item=(w-(columns-1)*8)/columns;final cards=[tile(Icons.people,'$clients','Clientes registrados'),tile(Icons.local_taxi,'$drivers','Conductores registrados'),tile(Icons.route,'$trips','Viajes totales'),tile(Icons.navigation,'$activeTrips','Viajes activos'),tile(Icons.pending_actions,'$pendingPayments','Pagos pendientes'),tile(Icons.payments,'$grossCup CUP','Viajes completados')];return Wrap(spacing:8,runSpacing:8,children:cards.map((x)=>SizedBox(width:item,child:x)).toList());});
}
