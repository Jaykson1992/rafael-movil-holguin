...
class _AdminPinScreenState extends State<AdminPinScreen>{
 ...
 Future<void> enter() async {
   ...
   }catch(_){if(mounted)setState((){tries++;error='No se pudo iniciar sesión. Verifica los datos o la conexión.';});}
...
class _DriverHomeState extends State<DriverHome>{final counter=TextEditingController();Timer? poll;bool syncing=false;final Set<String> serviceFilters={'ride','delivery','cargo'};@override void initState(){super.initState();appState.addListener(refresh);if(AppConfig.hasProductionServer){appState.syncPaymentSettings(appState.driverBackend()).catchError((_) {});loadInbox();poll=Timer.periodic(const Duration(seconds:5),(_)=>loadInbox());}}@override void dispose(){poll?.cancel();appState.removeListener(refresh);counter.dispose();super.dispose();}void refresh(){if(mounted)setState((){});}
 Future<void> loadInbox() async {if(syncing||appState.driverToken==null)return;syncing=true;try{final rows=await appState.driverBackend().driverInbox();final filtered=rows.where((x)=>serviceFilters.contains(x['serviceType']?.toString()??'ride')).toList();if(filtered.isNotEmpty){appState.trip=tripFromJson(filtered.first);appState.notifyListeners();}else if(appState.trip?.status!=TripStatus.completed){appState.trip=null;appState.notifyListeners();}}catch(_){}finally{syncing=false;}}
...
class _ServiceMetrics extends StatelessWidget {const _ServiceMetrics({required this.data,required this.completed});final Map<String,dynamic> data,completed;Widget item(String emoji,String title,String key)=>Expanded(child:Container(padding:const EdgeInsets.all(12),decoration:BoxDecoration(color:Colors.white,borderRadius:BorderRadius.circular(18),border:Border.all(color:const Color(0xffe8edf4))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(emoji,style:const TextStyle(fontSize:22)),Text('${data[key]??0}',style:const TextStyle(fontSize:22,fontWeight:FontWeight.w900)),Text(title,style:const TextStyle(fontWeight:FontWeight.w700)),Text('${completed[key]??0} completados',style:const TextStyle(fontSize:10,color:Colors.black54))])));@override Widget build(BuildContext c)=>Row(crossAxisAlignment:CrossAxisAlignment.start,children:[item('🚕','Viajes','ride'),const SizedBox(width:6),item('🛵','Delivery','delivery'),const SizedBox(width:6),item('📦','Carga','cargo')]);}
...
class AdminBusinessesPanel extends StatefulWidget{const AdminBusinessesPanel({super.key,required this.backend});final RestRafaelBackend backend;@override State<AdminBusinessesPanel> createState()=>_AdminBusinessesPanelState();}
class _AdminBusinessesPanelState extends State<AdminBusinessesPanel>{List<Map<String,dynamic>> rows=[];bool loading=false;final name=TextEditingController(),phone=TextEditingController(),category=TextEditingController(text:'Restaurante');
@override void initState(){super.initState();load();}@override void dispose(){name.dispose();phone.dispose();category.dispose();super.dispose();}
Future<void> load()async{setState(()=>loading=true);try{rows=await widget.backend.adminBusinesses();}catch(_){}if(mounted)setState(()=>loading=false);}
Future<void> add()async{if(name.text.trim().isEmpty||phone.text.trim().isEmpty)return;await widget.backend.createBusiness(name:name.text.trim(),phone:phone.text.trim(),category:category.text.trim().isEmpty?'Negocio':category.text.trim());name.clear();phone.clear();await load();}
@override Widget build(BuildContext c)=>Card(child:Padding(padding:const EdgeInsets.all(16),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[const Expanded(child:Text('Negocios y restaurantes',style:TextStyle(fontSize:19,fontWeight:FontWeight.w900))),IconButton(onPressed:loading?null:load,icon:const Icon(Icons.refresh))]),const Text('Socios para delivery. Rafael puede activarlos, pausarlos o eliminarlos.'),const SizedBox(height:12),TextField(controller:name,decoration:const InputDecoration(labelText:'Nombre del negocio',prefixIcon:Icon(Icons.storefront))),const SizedBox(height:8),TextField(controller:phone,keyboardType:TextInputType.phone,decoration:const InputDecoration(labelText:'Teléfono',prefixIcon:Icon(Icons.phone))),const SizedBox(height:8),TextField(controller:category,decoration:const InputDecoration(labelText:'Categoría',prefixIcon:Icon(Icons.restaurant))),const SizedBox(height:8),FilledButton.icon(onPressed:add,icon:const Icon(Icons.add_business),label:const Text('Agregar negocio')),if(loading)const LinearProgressIndicator(),...rows.map((b)=>ListTile(contentPadding:EdgeInsets.zero,leading:CircleAvatar(child:Icon(b['active']==false?Icons.store_mall_directory_outlined:Icons.store)),title:Text(b['name']?.toString()??'',style:const TextStyle(fontWeight:FontWeight.w800)),subtitle:Text('${b['category']??'Negocio'} · ${b['phone']??''}'),trailing:Wrap(spacing:0,children:[Switch(value:b['active']!=false,onChanged:(v)async{await widget.backend.setBusinessActive(b['id'].toString(),v);await load();}),IconButton(tooltip:'Eliminar',icon:const Icon(Icons.delete_outline),onPressed:()async{final ok=await showDialog<bool>(context:c,builder:(d)=>AlertDialog(title:const Text('Eliminar negocio'),content:Text('¿Eliminar ${b['name']}?'),actions:[TextButton(onPressed:()=>Navigator.pop(d,false),child:const Text('Cancelar')),FilledButton(onPressed:()=>Navigator.pop(d,true),child:const Text('Eliminar'))]));if(ok==true){await widget.backend.deleteBusiness(b['id'].toString());await load();}})]))) ])));}
...
class _AdminHomeState extends State<AdminHome>{
 ...
 @override Widget build(BuildContext c){
  ...
  return Scaffold(appBar:AppBar(title:const Text('Rafael Control'),actions:[if(AppConfig.hasProductionServer)IconButton(tooltip:'Actualizar',onPressed:loadingRemote?null:loadRemote,icon:const Icon(Icons.refresh)),if(AppConfig.hasProductionServer)IconButton(tooltip:'Cerrar sesión',onPressed:() async {try{await appState.adminBackend().logout();}catch(_){}appState.adminToken=null;if(c.mounted)Navigator.of(c).pushAndRemoveUntil(MaterialPageRoute(builder:(_)=>const RoleScreen()),(r)=>false);},icon:const Icon(Icons.logout))]),body:ListView(padding:const EdgeInsets.all(20),children:[
   ...
   if(d!=null)...[const SizedBox(height:14),const Text('Servicios reales',style:TextStyle(fontSize:20,fontWeight:FontWeight.w900)),const SizedBox(height:8),_ServiceMetrics(data:Map<String,dynamic>.from((d['serviceCounts'] as Map?)??{}),completed:Map<String,dynamic>.from((d['completedByService'] as Map?)??{}))],if(!AppConfig.hasProductionServer)SwitchListTile(value:appState.driverActive,onChanged:(v){appState.driverActive=v;appState.notifyListeners();},title:const Text('Conductor activo'),subtitle:const Text('Rafael puede pausar la cuenta')),
   ...
   if(!AppConfig.hasProductionServer)OutlinedButton(onPressed:appState.renew,child:const Text('Registrar pago: +30 días')),if(AppConfig.hasProductionServer) AdminBusinessesPanel(backend:appState.adminBackend()),
   if(AppConfig.hasProductionServer) AdminSubscriptionPaymentsPanel(backend:appState.adminBackend()),
