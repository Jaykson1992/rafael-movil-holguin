...
class _AdminDriversPanelState extends State<AdminDriversPanel> {
  ...
  @override Widget build(BuildContext context){return Card(child:Padding(padding:const EdgeInsets.all(12),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
    ...
    ...drivers.map((d){final sub=Map<String,dynamic>.from((d['subscription'] as Map?)??const{});final active=(sub['active']??true)==true;final busy=d['busy']==true;final online=d['online']==true;final availability=d['availability']?.toString()??(online?(busy?'busy':'available'):'offline');final paid=sub['paidUntil']?.toString();final paidDate=paid==null?null:DateTime.tryParse(paid)?.toLocal();final valid=paidDate!=null&&paidDate.isAfter(DateTime.now());final id=d['id']?.toString()??'';final phone=d['phone']?.toString().trim()??'';final vehicle=d['vehicle']?.toString()??'Auto';return Card(margin:const EdgeInsets.symmetric(vertical:6),child:ExpansionTile(
      leading:_avatar(d),title:Text(d['name']?.toString()??'Conductor',style:const TextStyle(fontWeight:FontWeight.w700)),subtitle:Row(children:[Icon(_vehicleIcon(vehicle),size:16),const SizedBox(width:5),Expanded(child:Text('$vehicle · ${availability=='busy'?'En viaje':availability=='available'?'En línea · Disponible':'Desconectado'}'))]),
      trailing:Icon(availability=='available'?Icons.radio_button_checked:availability=='busy'?Icons.local_taxi:Icons.radio_button_unchecked),children:[Padding(padding:const EdgeInsets.fromLTRB(16,0,16,14),child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[
        Text('Carnet: ${d['identity']??'-'}'),const SizedBox(height:4),Text('Teléfono: ${phone.isEmpty?'-':phone}'),const SizedBox(height:4),Text(valid?'Mensualidad hasta ${paidDate.month}/${paidDate.day}/${paidDate.year}':'Mensualidad vencida o pendiente'),
