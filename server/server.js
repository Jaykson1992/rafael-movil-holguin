...
app.post('/v1/auth/register',(req,res)=>{const {name,identity,role:requestedRole,vehicle,pin}=req.body;const r=requestedRole||'client';const normalizedVehicle=normVehicle(vehicle);if(!name||!identity||!validPin(pin)||!['client','driver'].includes(r)||(r==='driver'&&!VEHICLES.has(normalizedVehicle)))return res.status(400).json({error:'invalid_fields'});if(Object.values(db.users).some(u=>u.identity===identity))return res.status(409).json({error:'identity_exists'});const id=crypto.randomUUID();const u={id,name,identity,role:r,vehicle:r==='driver'?normalizedVehicle:null,pinHash:hashPin(pin),active:true,createdAt:now()};db.users[id]=u;if(r==='driver')db.drivers[id]={id,active:true,paidUntil:null};save();res.status(201).json(publicUser(u));});
...
app.get('/v1/admin/clients',auth,role('admin'),(req,res)=>{
  ...
  let data=Object.values(db.users).filter(u=>u.role==='client').map(u=>({...publicUser(u),trips:Object.values(db.trips).filter(t=>t.clientId===u.id).length}));
