...
function fresh(){return {users:{},sessions:{},trips:{},locations:{},drivers:{},businesses:{},paymentRequests:{},audit:[],settings:{monthlyFeeCup:1000,transfermovilCard:'',subscriptionDays:30}}}
let db=fresh();
...
app.post('/v1/trips',auth,role('client'),(req,res)=>{const {origin,destination,vehicle,offer,requestId}=req.body;const serviceType=['ride','delivery','cargo'].includes(String(req.body.serviceType))?String(req.body.serviceType):'ride';const details=String(req.body.details||'').trim().slice(0,500);const v=normVehicle(vehicle);const nOffer=Number(offer);if(!origin||!destination||!VEHICLES.has(v)||!Number.isFinite(nOffer)||nOffer<=0||nOffer>1000000)return res.status(400).json({error:'missing_fields'});if(requestId){const prior=Object.values(db.trips).find(t=>t.clientId===req.user.id&&t.requestId===String(requestId));if(prior)return res.status(200).json(prior);}const t={id:crypto.randomUUID(),requestId:requestId?String(requestId).slice(0,100):null,origin,destination,vehicle:v,offer:nOffer,serviceType,details,clientId:req.user.id,status:'searching',counterOffer:null,driverId:null,createdAt:now(),updatedAt:now()};db.trips[t.id]=t;save();res.status(201).json(t);});
...
function tripView(t,viewer){const out={...t};if(['assigned','arriving','inProgress'].includes(t.status)){if(viewer.role==='client'&&t.clientId===viewer.id&&t.driverId){const d=db.users[t.driverId];out.counterpart=d?{name:d.name,phone:d.phone||'',vehicle:d.vehicle||null,photoUrl:d.photoUrl||''}:null;}else if(viewer.role==='driver'&&t.driverId===viewer.id){const cl=db.users[t.clientId];out.counterpart=cl?{name:cl.name,phone:cl.phone||'',photoUrl:cl.photoUrl||''}:null;}}return out;}
app.get('/v1/trips/:id',auth,(req,res)=>{const t=db.trips[req.params.id];if(!t)return res.status(404).json({error:'not_found'});if(req.user.role==='client'&&t.clientId!==req.user.id)return res.status(403).json({error:'forbidden'});if(req.user.role==='driver'&&t.driverId&&t.driverId!==req.user.id)return res.status(403).json({error:'forbidden'});res.json(tripView(t,req.user));});
...
app.get('/v1/admin/businesses',auth,role('admin'),(_,res)=>{res.json(Object.values(db.businesses||{}).sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))));});
app.post('/v1/admin/businesses',auth,role('admin'),(req,res)=>{const name=String(req.body.name||'').trim().slice(0,100),phone=String(req.body.phone||'').trim().slice(0,40),category=String(req.body.category||'Negocio').trim().slice(0,60);if(!name||!phone)return res.status(400).json({error:'missing_fields'});const b={id:crypto.randomUUID(),name,phone,category,active:true,createdAt:now()};db.businesses[b.id]=b;audit(req.user,'business_created',{businessId:b.id,name:b.name});save();res.status(201).json(b);});
app.patch('/v1/admin/businesses/:id',auth,role('admin'),(req,res)=>{const b=db.businesses?.[req.params.id];if(!b)return res.status(404).json({error:'not_found'});if(typeof req.body.active==='boolean')b.active=req.body.active;if(req.body.name)b.name=String(req.body.name).trim().slice(0,100);if(req.body.phone)b.phone=String(req.body.phone).trim().slice(0,40);if(req.body.category)b.category=String(req.body.category).trim().slice(0,60);audit(req.user,'business_updated',{businessId:b.id,active:b.active});save();res.json(b);});
app.delete('/v1/admin/businesses/:id',auth,role('admin'),(req,res)=>{const b=db.businesses?.[req.params.id];if(!b)return res.status(404).json({error:'not_found'});delete db.businesses[req.params.id];audit(req.user,'business_deleted',{businessId:req.params.id,name:b.name});save();res.json({ok:true,id:req.params.id});});
app.get('/v1/admin/dashboard',auth,role('admin'),(_,res)=>{
  const trips=Object.values(db.trips), drivers=Object.values(db.drivers), users=Object.values(db.users), businesses=Object.values(db.businesses||{});
  ...
  const onlineClients=users.filter(u=>u.role==='client'&&u.active!==false&&Number.isFinite(Date.parse(u.lastSeenAt||0))&&Date.now()-Date.parse(u.lastSeenAt)<=2*60*1000).length; const onlineDrivers=users.filter(u=>u.role==='driver'&&freshLocation(db.locations[u.id])).length; const busyDrivers=users.filter(u=>u.role==='driver'&&freshLocation(db.locations[u.id])&&driverBusy(u.id)).length; const serviceCounts={ride:0,delivery:0,cargo:0};for(const t of trips){const k=['ride','delivery','cargo'].includes(t.serviceType)?t.serviceType:'ride';serviceCounts[k]++;}const completedByService={ride:0,delivery:0,cargo:0};for(const t of completed){const k=['ride','delivery','cargo'].includes(t.serviceType)?t.serviceType:'ride';completedByService[k]++;}res.json({clients:users.filter(u=>u.role==='client').length,onlineClients,drivers:users.filter(u=>u.role==='driver').length,onlineDrivers,busyDrivers,availableDrivers:Math.max(0,onlineDrivers-busyDrivers),activeDrivers:drivers.filter(d=>d.active!==false&&d.paidUntil&&Date.parse(d.paidUntil)>Date.now()).length,trips:trips.length,activeTrips,completedTrips:completed.length,grossCup,pendingPayments,monthlyFeeCup:db.settings.monthlyFeeCup,businesses:businesses.length,activeBusinesses:businesses.filter(b=>b.active!==false).length,serviceCounts,completedByService});
});
bootstrapAdmin();
